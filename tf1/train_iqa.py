...
def process_command_args():
    ...
    parser.add_argument('--train_list',type=str,default= 'train_info.txt',help='train TOMO list for read image.')
    parser.add_argument('--test_list', type=str, default= 'test_info.txt', help='test TOMO list for read image.')
    ...
    parser.add_argument('--pretrain_models_path',type=str,default="experiments/TOMO/rankiqa/"+'model.ckpt-4999')
    ...
    parser.add_argument('--test_step',type=int,default=5000)
...
def load_data(args):
    train_image_paths = []
    train_scores = []
    # iteration for generating train image
    iter_image=5
    # Read train images
    f = open(args.train_list, 'r')
    for line in f:
        base_image_path, base_image_score = line.strip("\n").split()

        train_image_paths.append(base_image_path)
        train_scores.append(base_image_score)
    f.close()
...
def train(args):
    ...
    with graph.as_default():
        ...
        # # placeholders for training TOMO
        ...
        train_data = Dataset(
            {'root_dir': args.data_dir, 'data_root': args.dataset, 'split': 'train_info', 'im_shape': [224, 224],
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for step in range(args.iter_max):
            ...
            if (step+1) % args.test_step == 0:
                ...
                #     logger.info('test_loader step/len(test_loader) :{}/{}'.format(i,test_epoch_step))
                #
