...
class VggNetModel(object):
    ...
    def inference(self, x, training=False):
        ...
        with tf.variable_scope("fc9") as scope:
            fc9w=tf.get_variable("weights", initializer=tf.truncated_normal([4096, self.num_levels], dtype=tf.float32, stddev=1e-1))
            fc9b = tf.get_variable('biases', initializer=tf.constant(1.0, shape=[self.num_levels], dtype=tf.float32))
            self.levels = tf.nn.bias_add(tf.matmul(fc7, fc9w), fc9b)
