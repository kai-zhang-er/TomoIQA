...
def mdsi(ref_image, pred_image, f, alpha):
...
def gray_mdsi(ref_image, pred_image, f, alpha):
...
if __name__ == '__main__':
    # main()
    image1=io.imread("TOMO/testset/0_0_0_0.bmp")
    rgb1=np.zeros((image1.shape[0],image1.shape[1],3),np.uint8)
    rgb1[:,:,0]=image1
    rgb1[:,:,1]=image1
    rgb1[:,:,2]=image1
    image2=io.imread("TOMO/testset/0_0_5_0.bmp")
    rgb2 = np.zeros((image2.shape[0], image2.shape[1], 3), np.uint8)
    rgb2[:, :, 0] = image2
    rgb2[:, :, 1] = image2
    rgb2[:, :, 2] = image2
    print("score=",gray_mdsi(image1,image2,f=0,alpha=0.6))
    print("score=", mdsi(rgb1, rgb2, f=0, alpha=0.6))