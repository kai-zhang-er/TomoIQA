...
from src.loss.EMD_loss import EMDLoss
...
def process_command_args():
    ...
    parser.add_argument('--data_dir',type=str,default="/gdrive/My Drive/pyIQA/",help='the root path of dataset')
    parser.add_argument('--train_list',type=str,default='live_train.txt',help='TOMO list for read image.')
    parser.add_argument('--test_list', type=str, default='live_test.txt', help='TOMO list for read image.')
    parser.add_argument('--ckpt_dir',type=str,default='experiments',help='the path of ckpt file')
    parser.add_argument('--logs_dir',type=str,default='experiments',help='the path of tensorboard logs')
    parser.add_argument('--vgg_models_path',type=str,default="experiments/vgg_models/"+'vgg16_weights.npz')
    ...
    parser.add_argument('--dataset',default='TOMO',type=str,choices=["LIVE", "TOMO", "tid2013"],help='datset choice')
    ...
    parser.add_argument('--iter_max',type=int,default=5000,help='the maxinum of iteration')
...
def train(args):
    ...
    with graph.as_default():
        ...
        imgs = tf.placeholder(tf.float32, [args.batch_size, args.crop_height, args.crop_width, 3])
        scores = tf.placeholder(tf.float32,[args.batch_size,5])
        dropout_keep_prob = tf.placeholder(tf.float32,[])
        lr = tf.placeholder(tf.float32, [])

        with tf.name_scope("create_models"):
            model = VggNetModel(num_classes=1,dropout_keep_prob=dropout_keep_prob)
            y_hat,y_level = model.inference(imgs,True)

        with tf.name_scope("create_loss"):
            rank_loss = Rank_loss()
            loss1 = rank_loss.get_rankloss(y_hat)
            loss2=EMDLoss().get_emd_loss(scores,y_level)
            loss=loss1+loss2
        ...
        summary_writer = tf.summary.FileWriter(os.path.join(args.logs_dir,'train/{}_{}'.format(args.exp_name,timestamp)),filename_suffix=args.exp_name)
        summary_test = tf.summary.FileWriter(os.path.join(args.logs_dir ,'test/{}_{}'.format(args.exp_name, timestamp)) , filename_suffix=args.exp_name)

        train_data = Dataset({'root_dir': args.data_dir, 'data_root': args.dataset, 'split': 'train_info', 'im_shape': [224, 224],'batch_size': args.batch_size})
        test_data = Dataset(
            {'root_dir': args.data_dir, 'data_root': args.dataset, 'split': 'test_info', 'im_shape': [224, 224],
             'batch_size': args.batch_size})

        model_checkpoint_path="/gdrive/My Drive/pyIQA/experiments/TOMO/rankiqa/model.ckpt-2499"
    with tf.Session(graph=graph) as sess:
        ...
        for step in range(args.iter_max):
            ...
            if (step+1) % args.test_step == 0:
                ...
                test_epoch_step = len(test_data.image_path)//test_data.batch_size+1
                test_loss = 0
